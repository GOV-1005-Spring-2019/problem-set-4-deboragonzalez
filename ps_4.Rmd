...
Note: Fixed the typo per Dylan's instruction. 
...
nc_election %>%
  ...
  # Need to create this total variable in order to calculate the weighted proportions of response by race.
  spread(key =  response, value = total, fill = 0) %>% 
  # Spread so that our chart shows 
  mutate(all = Dem + Rep + Und + `3`) %>% 
  mutate(Dem = Dem / all) %>% 
  mutate(Rep = Rep / all) %>% 
  mutate(Und = Und / all) %>% 
  # This chunk of code creates the proportions from the total. 
  select(-all, -`3`) %>% 
  # This diselects the all and third party columns from the table we are about to make.
  ungroup() %>% 
  # Will now explicitly ungroup so that I don't carry my 
  na_if(0) %>%
